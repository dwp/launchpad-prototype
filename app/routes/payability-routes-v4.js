...
function getNextIncompleteTask(stay) {
  ...
  const order = [
    { key: "type", url: "/payability/v4/stay/type" },
    { key: "startDate", url: "/payability/v4/stay/date-in" },
    { key: "endDate", url: "/payability/v4/stay/date-left" },
    { key: "address", url: "/payability/v4/stay/accommodation-lookup-name" },
    { key: "funding", url: "/payability/v4/stay/funding-type" },
    { key: "repay", url: "/payability/v4/stay/repay" },
    { key: "boarder", url: "/payability/v4/stay/accommodation-boarder" },
    { key: "similarInstitution", url: "/payability/v4/stay/similar-institution" },
    { key: "contact", url: "/payability/v4/stay/accommodation-contact-check" },
    { key: "fundingAddress", url: "/payability/v4/stay/funding-address-check" },
    { key: "fundingContact", url: "/payability/v4/stay/funding-contact-check" }
  ];
...
router.get('/payability/v4/stay/start', function (req, res) {
  const d = req.session.data;
  d.stays = d.stays || [];
  ...
  return res.redirect('/payability/v4/stay/task-list');
});
...
router.post('/hospital-check', function (req, res) {
  ...
  return res.redirect('/payability/v4/stay/start');
});
...
router.get('/payability/v4/stay/task-list-continue', function (req, res) {
  const d = req.session.data;
  const stay = d.stays[d.stayIndex];
  ...
  return res.redirect('/payability/v4/stay/check-answers');
});
...
router.post('/hospital-or-hospice', function(req, res) {
  ...
  if (v === "No") {
    return res.redirect("/payability/v4/stay/type");
  }
  return res.redirect("/payability/v4/stay/date-yesterday-check");
});
...
router.post('/yesterday-check', function(req, res) {
  ...
  if (t === "Hospital") {
    return res.redirect("payability/v4/stay/accommodation-lookup-name");
  }
  return res.redirect("payability/v4/stay/accommodation-boarder");
});
...
router.post('/still-there-check', function(req, res) {
  ...
  if (stillThere === "No") {
    return res.redirect("payability/v4/stay/date-left");
  }
...
router.post('/boarder-check', function(req, res) {
  ...
  if (v === "Yes") {
    return res.redirect("payability/v4/stay/accommodation-boarder-detail");
  }

  return res.redirect("payability/v4/stay/similar-institution");
});
...
router.post('/address-check', function(req, res) {
  ...
  if (v === "Yes") {
    return res.redirect("payability/v4/stay/accommodation-lookup-start");
  }

  return res.redirect("payability/v4/stay/accommodation-contact-check");
});
...
router.post('/accommodation-lookup-confirm', function(req, res) {
  ...
  if (addressRoute === "Detention in Legal Custody") {
    return res.redirect("payability/v4/stay/accommodation-lookup-confirm");
  }
  return res.redirect("payability/v4/stay/accommodation-lookup-confirm");
});
...
router.post('/confirm-address', function(req, res) {
  ...
  return res.redirect("payability/v4/stay/funding-type");
});
...
router.post('/persona-selection', function(req, res) {
  ...
  if (v === "Agent handling an ongoing claim") {
    return res.redirect("payability/v4/scenario");
  }

  else if (v === "Agent completing new claim stay end date") {
    return res.redirect("payability/v4/agent/index");
  }

  return res.redirect("payability/v4/stay/accommodation-hospital-check");
});
...
router.post('/funding-check', function(req, res) {
  ...
  if (persona === "Agent handling an ongoing claim") {
    return res.redirect("payability/v4/stay/funding-address-check");
  }
...
// --- Funding scenario ---
router.post('/funding-scenario-v4', function(req, res) {
  updateStayObject(req);
...
router.post('/funding-address-check', function(req, res) {
  ...
  if (v === "Yes") {
    return res.redirect("payability/v4/stay/funding-lookup-name");
  }

  return res.redirect("payability/v4/stay/funding-contact-select");
});
...
router.post('/funding-contact-check', function(req, res) {
  ...
  return res.redirect("payability/v4/person-record/stay-task-list");
});
...
router.post('/accommodation-contact-select', function(req, res) {
  ...
  return res.redirect('payability/v4/stay/accommodation-contact');
});
...
router.post('/funding-contact-select', function(req, res) {
  ...
  return res.redirect('payability/v4/stay/funding-contact');
});
...
router.get('/payability/v4/stay/:id/overview', function (req, res) {
  const d = req.session.data;
  ...
  res.render('payability/v4/person-record/stay-overview');
});
...
router.get('/payability/v4/stay/task-list', function (req, res) {
  const d = req.session.data;
  const stay = d.stays[d.stayIndex];
  ...
  res.render('payability/v4/person-record/stay-task-list', {
    stay,
    stays: d.stays
  });
});
...
router.get('/payability/v4/stay/continue', function (req, res) {
  const d = req.session.data;
  const stay = d.stays[d.stayIndex];
  ...
  if (next) {
    ...
    return res.redirect('/payability/v4/stay/check-all-answers');  // 🎉 everything complete
  }
});
...
router.get('/payability/v4/stay/pip-stay-complete', function(req, res) {
  ...
  res.render('payability/v4/stay/pip-stay-complete');
});
...
router.post('/save-stay', function(req, res) {
  ...
  if (persona === "Agent completing new claim stay end date") {
  return res.redirect('/payability/v4/stay/pip-reg-stay-task-list-complete');
 } else {
    return res.redirect('/payability/v4/stay/accommodation-lookup-name');
  }
})
